...
client_library_name = cmake_helpers.client_library_name(module_name)
%>\
#---------------------------------------------------------------------
...
add_library(${service_library_name}
  ${generate_sources.proto_srcs}
  ${generate_sources.grpc_srcs}
% for output_file in cmake_helpers.list_service_output_cpp_files(config, r"${service_output_dir}"):
  ${output_file}
% endfor
...
<%
client_output_files = cmake_helpers.list_client_output_cpp_files(config, r"${service_output_dir}")
%>\
% if any(client_output_files):
...
target_include_directories(${client_library_name}
  PUBLIC 
    ${r"${proto_srcs_dir}"} 
  PRIVATE
    "./"
